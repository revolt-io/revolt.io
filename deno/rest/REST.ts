...
import type { APIRoutes } from 'https://deno.land/x/revolt_api@0.4.0/routes.ts'
import { APIRequest, APIRequestOptions, RequestMethod } from './APIRequest.ts'
...
export class REST {
    ...
    get<Path extends GetRoutes['path']>(path: Path, options: APIRequestOptions = {}): Promise<any> {
    ...
    post<Path extends PostRoutes['path']>(path: Path, options: APIRequestOptions = {}): Promise<any> {
        return this.request(new APIRequest(this.options.url + path, this.getOptions(options, RequestMethod.Post)))
    }

    put<Path extends PutRoutes['path']>(path: Path, options: APIRequestOptions = {}): Promise<any> {
        return this.request(new APIRequest(this.options.url + path, this.getOptions(options, RequestMethod.Put)))
    }

    patch<Path extends PatchRoutes['path']>(path: Path, options: APIRequestOptions = {}): Promise<any> {
